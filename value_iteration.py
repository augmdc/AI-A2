...
def transition_probability(possible_actions, noise_prob, state, action_performed, state_prime):
    for action in possible_actionactions: # For each intended action
        expected_value = 0
        ...
        for possible_action in actions: # Four possible outcomes
            if possible_action == action: # This is the probability that the intended action is executed successfully
              return 1 - noise_prob # if noise_prob is 0.2, there's an 80% chance the intended action will occur.
            else:
